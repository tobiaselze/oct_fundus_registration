function [vesselness,Mm] = vesselness2D(I, sigmas, spacing, tau, brightondark)
...
I = single(I);
Mm=zeros(size(I)).*-10000; % para buscar por maximos
...
for j = 1:length(sigmas)
    ...
    [~, Lambda2,G2] = imageEigenvalues(I,sigmas(j),spacing,brightondark); 
    if brightondark == true
        Lambda2 = -Lambda2;
    end  
    
    % proposed filter at current scale for lambda2
    Lambda3 = Lambda2;
    ...
    % Keeps the maximum over scales (Mm)
    G2=G2./sigmas(j);
...
function [Lambda1, Lambda2,G] = imageEigenvalues(I,sigma,spacing,brightondark)
% calculates the two eigenvalues for each voxel in a volume

% Calculate the 2D hessian
[Hxx, Hyy, Hxy, Dx, Dy] = Hessian2D(I,sigma,spacing);
...
function [Dxx, Dyy, Dxy, Dx, Dy] = Hessian2D(I,Sigma,spacing)
...
% 
% [Dxx, Dyy, Dxy, Dx, Dy] = Hessian2D(I,Sigma,spacing)
% 
% inputs,
